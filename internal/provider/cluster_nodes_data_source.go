...
var (
	_ datasource.DataSource              = &pveClusterNodesDataSource{}
	_ datasource.DataSourceWithConfigure = &pveClusterNodesDataSource{}
)

// NewPveClusterNodesDataSource returns the data source implementation.
func NewPveClusterNodesDataSource() datasource.DataSource {
	return &pveClusterNodesDataSource{}
}

// pveClusterNodesDataSource lists the nodes in the cluster (GET /nodes).
type pveClusterNodesDataSource struct {
	client *pveclient.Client
}

// pveClusterNodesDataSourceModel is the Terraform-facing shape.
type pveClusterNodesDataSourceModel struct {
	ID     types.String                         `tfsdk:"id"`
	Node   types.String                         `tfsdk:"node"`
	Status types.String                         `tfsdk:"status"`
	Nodes  []pveClusterNodesDataSourceNodeModel `tfsdk:"nodes"`
}

// pveClusterNodesDataSourceNodeModel mirrors the per-row schema of the
// cluster node list. Field names line up with the pveclient struct.
type pveClusterNodesDataSourceNodeModel struct {
	Node           types.String  `tfsdk:"node"`
	Status         types.String  `tfsdk:"status"`
...
func (d *pveClusterNodesDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_" + TypeNamePveClusterNodes
}

// Schema implements datasource.DataSource.
func (d *pveClusterNodesDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *pveClusterNodesDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *pveClusterNodesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data pveClusterNodesDataSourceModel
	...
	if err != nil {
		resp.Diagnostics.AddError("Error reading pve_cluster_nodes", fmt.Sprintf("listing cluster nodes: %s", err))
		return
	}
	nodeFilter := data.Node.ValueString()
	statusFilter := data.Status.ValueString()
	rows := make([]pveClusterNodesDataSourceNodeModel, 0, len(nodes))
	for _, n := range nodes {
		...
		rows = append(rows, pveClusterNodesDataSourceNodeModel{
	...
	data.ID = types.StringValue("pve_cluster_nodes")
	data.Nodes = rows
