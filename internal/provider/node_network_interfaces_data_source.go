		...
		Attributes: map[string]schema.Attribute{
			...
			"interfaces": schema.ListNestedAttribute{
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						"node":         schema.StringAttribute{Computed: true, MarkdownDescription: "Name of the PVE node the interface belongs to."},
						...
						"cidr":         schema.StringAttribute{Computed: true, MarkdownDescription: "IPv4 or IPv6 address in CIDR notation, when configured."},
						"address":      schema.StringAttribute{Computed: true, MarkdownDescription: "Plain IP address, when configured."},
						"gateway":      schema.StringAttribute{Computed: true, MarkdownDescription: "Default gateway, when configured."},
						"mtu":          schema.Int64Attribute{Computed: true, MarkdownDescription: "Maximum transmission unit in bytes, when set."},
						"comments":     schema.StringAttribute{Computed: true, MarkdownDescription: "Free-form comment shown in the UI, when set."},
						"method":       schema.StringAttribute{Computed: true, MarkdownDescription: "Address assignment method: `static`, `dhcp`, `manual`, or `loopback`."},
						"bridge_ports": schema.StringAttribute{Computed: true, MarkdownDescription: "Comma-separated member interfaces (bridge type only)."},
						"bridge_stp":   schema.BoolAttribute{Computed: true, MarkdownDescription: "Whether spanning tree protocol is enabled (bridge type only)."},
						"bridge_fd":    schema.Int64Attribute{Computed: true, MarkdownDescription: "Bridge forward delay in seconds (bridge type only)."},
						"vlan_id":      schema.Int64Attribute{Computed: true, MarkdownDescription: "VLAN tag (vlan type only)."},
						"ovs_bridge":   schema.StringAttribute{Computed: true, MarkdownDescription: "OVS bridge the interface belongs to, when applicable."},
						"ovs_type":     schema.StringAttribute{Computed: true, MarkdownDescription: "OVS port type. One of `internal`, `native`, `trunk`, `access`, or `patch`."},
						"ovs_options":  schema.MapAttribute{Computed: true, ElementType: types.StringType, MarkdownDescription: "Extra OVS options (e.g. `tag=10`)."},
						"bond_mode":    schema.StringAttribute{Computed: true, MarkdownDescription: "Linux kernel bonding driver mode (bond type only)."},
						"bond_primary": schema.StringAttribute{Computed: true, MarkdownDescription: "Primary bond slave (bond type only)."},
						"slaves":       schema.ListAttribute{Computed: true, ElementType: types.StringType, MarkdownDescription: "Member interfaces of a bond."},
					},
				},
			},
		},
