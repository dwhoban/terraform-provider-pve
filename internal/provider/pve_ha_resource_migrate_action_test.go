...
// TestPveHaResourceMigrate_SchemaAndMetadata covers the HA migrate
// action's type name and schema shape.
func TestPveHaResourceMigrate_SchemaAndMetadata(t *testing.T) {
	a := NewPveHaResourceMigrateAction()
	ctx := context.Background()
	metaResp := &action.MetadataResponse{}
	a.Metadata(ctx, action.MetadataRequest{ProviderTypeName: "pve"}, metaResp)
	if metaResp.TypeName != "pve_"+TypeNamePveHaResourceMigrate {
		t.Fatalf("TypeName = %q, want pve_%s", metaResp.TypeName, TypeNamePveHaResourceMigrate)
	}
