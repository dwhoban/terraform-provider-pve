...
// TestPveNodeMigrateAll_SchemaAndMetadata covers the migrate_all action's
// type name and schema shape.
func TestPveNodeMigrateAll_SchemaAndMetadata(t *testing.T) {
	a := NewPveNodeMigrateAllAction()
	ctx := context.Background()
	metaResp := &action.MetadataResponse{}
	a.Metadata(ctx, action.MetadataRequest{ProviderTypeName: "pve"}, metaResp)
	if metaResp.TypeName != "pve_"+TypeNamePveNodeMigrateAll {
		t.Fatalf("TypeName = %q, want pve_%s", metaResp.TypeName, TypeNamePveNodeMigrateAll)
	}
	schemaResp := &action.SchemaResponse{}
	a.Schema(ctx, action.SchemaRequest{}, schemaResp)
	for _, key := range []string{"node", "target", "max_workers", "vms", "with_local_disks"} {
